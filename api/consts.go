...
const SQL_VIEW_CANDIDATE_VACANCIES = `select v.id,
		...
		CONCAT(platforms.name, ' ("', 
			(select name from public.seniorities where id=seniority_id), '")') as platform,
...
FROM vacancies v JOIN companies on (v.company_id=companies.id)
	...
	JOIN public.platforms ON (v.platform_id = platforms.id)
	JOIN public.status_for_vacs sv on (vc.status = sv.id)
...
const SEND_CV_COMPANIES_SQL = `SELECT id as comp_id, c.name, send_details,
  (select json_agg(json_build_object('email', t.email, 'id',t.id, 'all_platforms', t.platforms is null,
           'platforms', t.platforms, 'name', t.name))
             from contacts t 
           WHERE t.company_id = c.id AND (platforms is null OR platforms && ANY($1))) as contacts,
